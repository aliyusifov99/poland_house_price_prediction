...
def rent_price_page():
    ...
    ownership = st.selectbox("Ownership", ['condominium'])
    ...
    if st.button("Predict Rent Price"):
        if floor > floorCount:
            ...
            url = "https://fastapi-backend-2vop66ad7a-uc.a.run.app/api/predict_rent"
            input_data = {
                "city": city,
                "type": apt_type,
            ...
            try:
                ...
                st.success(f"Predicted Rent Price: PLN {round(result['predicted_rent'], 0)}")
